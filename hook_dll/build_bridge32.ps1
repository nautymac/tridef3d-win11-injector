...
$srcDir = $PSScriptRoot
$outDll = "$srcDir\bridge_d3d9.dll"

# x86 lib package name differs from the x64 one we used before
...
$argsList = @(
  ...
  "$srcDir\bridge_d3d9.cpp",
  "/Fe:$outDll",
  "/link"
) + $libpaths + @(
  "d3d9.lib", "user32.lib", "kernel32.lib"
)

Write-Output "=== Compiling (x86) ==="
& $cl @argsList
